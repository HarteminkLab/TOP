...
#' @param ref.size Reference library size (default: 1e8).
...
merge_normalize_bin_transform_counts <- function(counts_files,
  ...
  # MILLIPEDE binning and transform counts
  bins.df <- bin_transform_counts(normalized_countmatrix, bin.method, transform)
...
#' @description Count genomic cleavage (5' end) from DNase or ATAC-seq bam files
#' using the \code{genomecov} tool from \code{bedtools}.
#' Then save the output in Bigwig format using
#' the \code{bedGraphToBigWig} tool
#' (and \code{bedSort} if available) from UCSC.
...
#' @param sites.df A data frame containing candidate sites
...
#' on the reverse (minus) strand
#'
#' @param sites.df A data frame containing candidate sites
#' @param fwd_matrix_file File for count matrix on forward strand
#' @param rev_matrix_file File for count matrix on reverse strand
#' @importFrom  data.table fread fwrite
#'
