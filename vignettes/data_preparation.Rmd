...
```{r motif-file}
motif_file <- system.file('example', 'MA0139.1.meme', package='TOP')
```
...
fimo_motif_scan(motif_file, sequence_file='hg38.fa', thresh='1e-5', 
                max_strand=TRUE, outname='MA0139.1_1e-5.fimo.txt',
                fimo_path='fimo')
```
...
bam_sort_index_stats('K562.ATAC.bam', sort=FALSE, index=TRUE, idxstats=TRUE)
```

Next, we count the cleavages along the genome, and save the genome counts 
in BigWig format. 
...
We need [`bedtools`](https://bedtools.readthedocs.io/en/latest/) 
...
combined_data <- readRDS('../inst/extdata/processed_data/CTCF_MA0139.1_1e-5.K562.ATAC.M5.combined.data.rds')
```
...
# chrom_size_file: file of genome sizes by chromosomes.
# ref.size: normalize to ChIP-Seq reference library size (default: 20 million).
...
combined_data <- readRDS('../inst/extdata/processed_data/CTCF_MA0139.1_1e-5.K562.ATAC.M5.ChIP.combined.data.rds')
```
