...
You can set option `max_strand=TRUE`, which will only report the match 
...
Save output in a text file `MA0139.1_1e-5.fimo.txt`, 
...
Obtain the candidate sites and save to a text file (`MA0139.1_1e-5.candidate_sites.txt`)
...
in BigWig files. 
This step could take a long time, but it only needs to be done once. 
The BigWig files will be used later for extracting the cuts 
...
You can shift ATAC-seq reads by setting `shift_ATAC=TRUE`. 
This will shift the reads aligned to the + strand by +4 bp, and shift
reads aligned to the - strand by -5 bp,
as in [Buenrostro et al. 2013](https://doi.org/10.1038/nmeth.2688). 
Yet, we found the shifting of ATAC-seq reads have little impact on the results, 
mostly due to our binning approach being robust to 
the small-scale positional shifts or digestion bias. 

```{r count_genome_coverage, eval=FALSE}
# bam_file: sorted BAM file.
# chrom_size_file: file of genome sizes by chromosomes.
# shift_ATAC: If TRUE, shift ATAC-sesq reads aligned to the + strand by +4 bp, 
# and shift reads aligned to the - strand by -5 bp. 
# outdir: directory for saving the BigWig files of genome counts.
...
# or use count_genome_cuts_bedtools() if you have bedtools installed (this is often faster).
count_genome_cuts_bedtools(bam_file='K562.ATAC.bam', 
                           chrom_size_file='hg38.chrom.sizes', 
                           shift_ATAC=TRUE,
                           outdir='processed_data',
                           outname='K562.ATAC.bam',
