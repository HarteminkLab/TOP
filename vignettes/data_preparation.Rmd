...
The format of the input data frame (see example data below):
...
It could be quantitative TF occupancy (asinh transformed ChIP-seq read counts) or 
...
Here we only use one replicate sample (`ENCFF534DCE.bam`) as an example, 
and rename the file as `K562.ATAC.bam`.
...
You can also replace ChIP-seq counts with signal values 
...
We recommend writing a Snakemake pipeline to automate the whole procedure. 
