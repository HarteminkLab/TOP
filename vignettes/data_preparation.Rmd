...
Here we show an example for finding motif matches of CTCF. 
...
# count_matrix: DNase or ATAC count matrix
...
Make a data frame with PWM scores, DNase (or ATAC) bins. 
...
colnames(combined_data.df) <- c(colnames(sites.df), paste0('bin', 1:ncol(bins.df)))
```
...
Combine PWM scores, DNase (or ATAC) bins, and ChIP-seq counts.
```{r combine-data-with-chip, eval=FALSE}
combined_data.df <- data.frame(sites.df, bins.df, sites_chip.df$chip)
colnames(combined_data.df) <- c(colnames(sites.df), paste0('bin', 1:ncol(bins.df)), 'chip')
```
